...
To answer this question, I first learn visual covariance so visual odometry can reject unreliable correspondences and weight residuals by predicted reliability.
Because visual evidence alone is fragile under high dynamics and sensor-specific inertial effects, I then extend uncertainty learning to inertial sensing through learned preintegration uncertainty and observability-aware inertial representations.
With both modalities producing metrics-aware uncertainty, I next design tuning-free visual-inertial initialization and calibration that fuse constraints by calibrated reliability rather than hand-tuned rules.
Once this calibrated uncertainty is trusted for estimation, I use the same signal as a deployment policy to guide quantization and token merging, so efficiency becomes a consequence of reliability-aware perception rather than a disconnected objective.

\medskip
\noindent\textbf{Metrics-Aware Covariance for Stereo Visual Odometry.}

I developed a stereo visual odometry system~\cite{anonymous2025macvo} that learns metrics-aware covariance from visual features, replacing hand-tuned geometric noise models as the first step of the uncertainty pipeline.
...
To address the remaining dynamics gap, I extend uncertainty learning to IMU preintegration, where fixed white-noise parameters fail to capture platform vibration, bias drift, and motion-dependent stochasticity.
